...
def run_consumer():
    ...
    def callback(msg_str: str):
        shipment_id, sent_date = process_order_placed(msg_str)

        if shipment_id is None or sent_date is None:
            return
        
        sent_date_obj = datetime.strptime(sent_date, "%Y-%m-%d").date()
        delivered_date = sent_date_obj + timedelta(days=5)
        new_event = ShipmentDeliveredEvent(shipment_id=shipment_id, delivered_date=delivered_date)
